 ...
 * flooring, thus if by chance the index is 0, we will get error. If we found nums[mid] > nums[mid+1], then mid may be the index of peak,
 ...
 * thus we will search right part by start = mid + 1. Why we can move start like mid +1? because we are sure there are still points after
